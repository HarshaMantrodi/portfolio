...
export default function Certifications() {
  return (
    <Section id="certifications" label="05 — Certifications" title="Credentials"
      subtitle="Verified certifications from Stanford, Google Cloud, AWS, and more." alt>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {CERTIFICATIONS.map((cert, i) => {
          ...
          return (
            ...
            >
              <motion.div
                className="absolute inset-0 pointer-events-none"
                style={{ background: `radial-gradient(ellipse 80% 60% at 20% 0%, ${cert.accent}08, transparent)` }}
                animate={{ opacity: 0 }}
                whileHover={{ opacity: 1 }}
                transition={{ duration: 0.3 }}
              />
              <div className="w-12 h-12 rounded-xl flex items-center justify-center text-xl flex-shrink-0 relative z-10"
                style={{ background: `${cert.accent}15`, border: `1px solid ${cert.accent}30` }}>
                {cert.icon}
              </div>
              <div className="relative z-10 min-w-0">
